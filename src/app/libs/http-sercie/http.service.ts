...
export class HttpService {
  ...
  public postJson<Input, Result>(
    ...
    // let webToken: string = localStorage.getItem('LOCAL_Sweb_token') ?? '';
    //
    // if (webToken) {
    //   webToken = webToken.slice(1, webToken.length - 1);
    // }

    // ob.appendHeader('token', webToken);

    ob.appendHeader('Content-Type', 'application/json');

    ob.appendParamsFromKeyValue(keyValue);

    return this.http.post<Result>(this.url(urlSuffix) , input, {
      observe: 'response',
      responseType: 'json',
      headers: ob.headers,
      params: ob.params,
  ...
  public postJson0<Input, Result>(
    ...
    return this.http.post<Result>(this.url(urlSuffix),
      {
        'useParamsFromBody': true,
        'params_Lr1oSgwPR8': keyValue,
        'body_o1nhHUG480': input,
      },
      {
        observe: 'response',
        responseType: 'json',
